...
class Assignment(db.Model):
    ...
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal):
        ...
        # Check if a teacher is grading the assignment
        if auth_principal.teacher_id:
            assertions.assert_valid(assignment.teacher_id == auth_principal.teacher_id, 
                                    f'This assignment was supposed to be evaluated by teacher with ID {assignment.teacher_id}.')
            assertions.assert_valid(grade is not None, 'Assignment with an empty grade cannot be graded.')
            assertions.assert_valid(assignment.state == AssignmentStateEnum.SUBMITTED, 
                                    'Only a submitted assignment can be graded.')
        
        # Check if a principal is grading the assignment
